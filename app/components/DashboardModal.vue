    ...
    <div class="bg-white dark:bg-[#1E293B]w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden relative flex flex-col max-h-[90vh]">
      ...
      <div class="p-6 sm:p-8 overflow-y-auto flex-1 space-y-8">
          ...
          <div class="bg-white dark:bg-[#1E293B]border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm overflow-x-auto">
            ...
            <div class="min-w-[350px] pt-4">
              ...
              <div class="flex justify-between w-full mt-3 gap-2 sm:gap-6">
                <div v-for="salesData in selectedItem.annualSales" :key="'sales-label-'+salesData.year" class="flex-1 text-center text-[10px] sm:text-xs font-bold text-gray-500">{{ salesData.year }}년</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="selectedItem?.priceHistory && selectedItem.priceHistory.length > 0">
          ...
          <div class="bg-white dark:bg-[#1E293B]border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm overflow-x-auto">
            ...
            <div class="min-w-[350px] pt-4">
              <div class="relative h-48 sm:h-56 w-full border-b border-gray-200 flex items-end justify-between gap-2 sm:gap-6">
                ...
                <div v-for="(price, index) in selectedItem.priceHistory" :key="price.year" class="flex-1 flex justify-center items-end h-full gap-1 sm:gap-2 relative z-10">
                  <div class="relative w-1/3 sm:w-8 bg-sky-500/80 rounded-t-sm flex justify-center hover:brightness-110 transition-all" :style="{ height: `${(price.wholesale / maxPrice) * 100}%` }">
                    <span class="absolute -top-5 text-[9px] sm:text-[11px] font-black text-blue-600 w-max">{{ price.wholesale.toLocaleString() }}</span>
                  </div>
                  <div class="relative w-1/3 sm:w-8 bg-blue-800/80 rounded-t-sm flex justify-center hover:brightness-110 transition-all" :style="{ height: `${(price.retail / maxPrice) * 100}%` }">
                    <span class="absolute -top-5 text-[9px] sm:text-[11px] font-black text-gray-800 w-max">{{ price.retail.toLocaleString() }}</span>
                  </div>
              ...
              <div class="flex justify-between w-full mt-3 gap-2 sm:gap-6">
                <div v-for="price in selectedItem.priceHistory" :key="'label-'+price.year" class="flex-1 text-center text-[10px] sm:text-xs font-bold text-gray-500">{{ price.year }}년</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="selectedItem?.customAnalyses && selectedItem.customAnalyses.length > 0" class="pt-6 border-t dark:border-slate-700/50">
          <div class="flex items-center justify-between">
            ...
            <button @click="showCustomDetails = true" class="px-4 py-1.5 bg-blue-100 text-blue-700 text-xs font-bold rounded-lg hover:bg-blue-200 transition">
              상세 내용 확인
            </button>
          </div>
        </div>
      ...
      <div v-if="showCustomDetails" class="absolute inset-0 z-50 bg-white/95 backdrop-blur-sm flex flex-col overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto p-6 sm:p-8 space-y-4">
          <div v-for="(analysis, idx) in selectedItem.customAnalyses" :key="idx" class="bg-sky-50/50 rounded-2xl p-5 border border-sky-100">
