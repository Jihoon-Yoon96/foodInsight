<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity p-0 sm:p-4">
    <div class="bg-white dark:bg-[#1E293B] w-full max-w-3xl rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden relative flex flex-col max-h-[85vh] sm:max-h-[90vh] transition-colors duration-300">

      <div class="px-5 sm:px-8 py-4 sm:py-6 border-b border-gray-100 dark:border-slate-700/50 flex justify-between items-start bg-blue-50/30 dark:bg-[#0F172A] shrink-0 transition-colors">
        <div class="min-w-0 pr-4">
          <span class="px-2 py-0.5 sm:px-2.5 sm:py-1 bg-sky-100 dark:bg-sky-900/30 text-sky-700 dark:text-sky-400 text-[10px] sm:text-xs font-black rounded-md uppercase tracking-wide transition-colors">AI 분석 리포트</span>
          <h2 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white mt-1 sm:mt-2 truncate transition-colors">{{ selectedItem?.PRDLST_NM }}</h2>
          <div class="flex items-center gap-3 mt-1 sm:mt-2">
            <p class="text-xs sm:text-sm text-gray-500 dark:text-slate-400 font-medium truncate transition-colors">{{ selectedItem?.BSSH_NM }}</p>
            <a v-if="reportData?.isCoupangAvailable" :href="reportData.coupangLink" target="_blank" class="inline-flex px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-[10px] sm:text-xs font-bold rounded hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors items-center gap-1 shadow-sm">
      ...
      <div class="p-5 sm:p-8 overflow-y-auto flex-1 scroll-smooth" ref="modalContentRef">
        <div v-if="isLoading" class="flex flex-col items-center justify-center py-16 sm:py-20">
          <div class="animate-spin rounded-full h-10 w-10 sm:h-12 sm:w-12 border-b-4 border-blue-600 dark:border-blue-500 mb-4"></div>
          <p class="text-blue-700 dark:text-blue-400 font-bold animate-pulse text-sm sm:text-base text-center transition-colors">AI가 제품 정보를 분석 중입니다...</p>
        </div>
        ...
        <div v-else-if="reportData" class="space-y-6 sm:space-y-8">
          <div>
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3 sm:gap-0 mb-3">
              ...
              <button v-if="!isInputActive" @click="toggleInput" class="px-3 py-1.5 bg-white dark:bg-[#1E293B] border border-blue-200 dark:border-blue-500/30 text-blue-600 dark:text-blue-400 text-xs font-bold rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors flex items-center justify-center gap-1 shadow-sm w-full sm:w-auto">
                추가 분석 요청하기
              </button>
            </div>
              ...
              <div class="min-w-[350px] pt-4">
                <div class="relative h-40 sm:h-48 w-full border-b border-gray-200 dark:border-slate-700/50 flex items-end justify-between gap-2 sm:gap-6 transition-colors">
                  <svg class="absolute inset-0 w-full h-full pointer-events-none overflow-visible z-20" viewBox="0 0 100 100" preserveAspectRatio="none">
                    <polyline :points="getSalesPolylinePoints(reportData)" fill="none" class="stroke-blue-900 dark:stroke-blue-400 transition-colors" stroke-width="2" vector-effect="non-scaling-stroke" />
                  </svg>
                ...
                <div class="flex justify-between w-full mt-3 gap-2 sm:gap-6">
                  <div v-for="salesData in reportData.annualSales" :key="'sales-label-'+salesData.year" class="flex-1 text-center text-[10px] sm:text-xs font-bold text-gray-500 dark:text-slate-400 transition-colors">{{ salesData.year }}년</div>
                </div>
              </div>
            </div>
          </div>

          <div class="bg-white dark:bg-[#0F172A] border border-gray-200 dark:border-slate-700/50 rounded-2xl overflow-hidden shadow-sm transition-colors">
            <div class="px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 dark:bg-[#1E293B] border-b border-gray-100 dark:border-slate-700/50 transition-colors">
              <p class="text-gray-800 dark:text-slate-300 text-[11px] sm:text-sm font-bold tracking-wider uppercase transition-colors">최근 4년 예상 도소매가 추이</p>
            </div>
            <div class="overflow-x-auto">
              <table class="w-full text-xs sm:text-sm text-left min-w-[300px]">
                <thead class="bg-white dark:bg-[#1E293B] text-gray-400 dark:text-slate-400 border-b border-gray-100 dark:border-slate-700/50 transition-colors">
                <tr>
                ...
                <tr v-for="price in reportData.priceHistory" :key="price.year" class="hover:bg-blue-50/30 dark:hover:bg-slate-800/50 transition-colors">
                  <td class="px-4 sm:px-6 py-3 sm:py-4 font-bold text-gray-700 dark:text-slate-300 transition-colors">{{ price.year }}년</td>
                  <td class="px-4 sm:px-6 py-3 sm:py-4 text-right font-bold text-blue-600 dark:text-blue-400 transition-colors">{{ price.wholesale.toLocaleString() }}원</td>
                  <td class="px-4 sm:px-6 py-3 sm:py-4 text-right font-bold text-gray-900 dark:text-white transition-colors">{{ price.retail.toLocaleString() }}원</td>
                </tr>
                </tbody>
              </table>
            </div>
          ...
          <div v-if="customAnalyses.length > 0 || isCustomLoading" class="pt-6 border-t border-gray-100 dark:border-slate-700/50 transition-colors">
            ...
            <div class="space-y-4">
              <div v-for="(analysis, index) in customAnalyses" :key="index" class="bg-sky-50/50 dark:bg-[#0F172A] rounded-2xl p-5 border border-sky-100 dark:border-slate-700/50 transition-colors">
              ...
              <div v-if="isCustomLoading" class="bg-gray-50 dark:bg-[#0F172A] rounded-2xl p-8 flex flex-col items-center justify-center border border-gray-100 dark:border-slate-700/50 transition-colors">
                <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 dark:border-blue-500 mb-3"></div>
                <p class="text-blue-700 dark:text-blue-400 font-bold text-sm animate-pulse transition-colors">AI가 추가 분석을 진행 중입니다...</p>
              </div>
            </div>

            <div v-if="customAnalyses.length > 0 && !isCustomLoading" class="mt-4 text-right">
              <button @click="saveCustomAnalysis" class="px-5 py-2.5 bg-gray-900 dark:bg-slate-700 text-white text-xs font-bold rounded-lg hover:bg-gray-800 dark:hover:bg-slate-600 transition shadow-sm">
                분석 내용 저장
              </button>
            </div>
          </div>
      ...
      <div class="px-5 sm:px-8 py-4 sm:py-5 bg-gray-50 dark:bg-[#0F172A] border-t border-gray-100 dark:border-slate-700/50 flex flex-col sm:flex-row justify-end items-center gap-3 shrink-0 transition-colors">

        <div v-if="isInputActive" class="w-full flex items-center gap-2 bg-white dark:bg-[#1E293B] border border-blue-200 dark:border-slate-600 rounded-xl px-2 py-1.5 shadow-inner flex-1 relative transition-colors">
          <input
              v-model="userPrompt"
              @keyup.enter="sendCustomPrompt"
              type="text"
              placeholder="AI에게 무엇이든 물어보세요 (예: 경쟁사 강점)"
              class="flex-1 bg-transparent text-sm outline-none px-2 py-1 text-gray-800 dark:text-white placeholder:text-gray-400 dark:placeholder:text-slate-500 transition-colors"
          >
          <button @click="sendCustomPrompt" :disabled="isCustomLoading" class="p-1.5 bg-blue-600 dark:bg-blue-500 text-white rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition disabled:opacity-50">
          ...
          <button @click="toggleInput" class="absolute -top-3 -right-2 bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-slate-600 text-gray-400 dark:text-slate-400 hover:text-gray-600 dark:hover:text-white rounded-full p-1 shadow-sm transition">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>

        <button v-if="!isInputActive" @click="toggleInput" class="w-full sm:w-auto px-6 py-2.5 bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-slate-600 text-gray-700 dark:text-slate-300 text-sm font-bold rounded-xl hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors shadow-sm shrink-0">
          추가 분석 요청하기
        </button>

        <button @click="$emit('close')" class="w-full sm:w-auto px-6 py-2.5 bg-blue-900 dark:bg-slate-700 text-white text-sm font-bold rounded-xl hover:bg-blue-600 dark:hover:bg-slate-600 transition-colors shadow-md shrink-0">
          닫기
        </button>
      </div>
