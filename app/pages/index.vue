  ...
  <div class="min-h-screen bg-[#F7F9F7] flex flex-col items-center justify-center px-4 font-sans">
    ...
    <div class="w-full max-w-5xl bg-white rounded-2xl shadow-2xl flex items-stretch overflow-hidden border border-emerald-100">
      ...
      <div class="flex-1 border-r border-gray-100 p-4 md:p-6 group focus-within:bg-emerald-50 transition">
        <label class="block text-xs font-bold text-emerald-600 mb-1 uppercase tracking-wider">원재료명</label>
        <input
            v-model="searchForm.rawMaterial"
            type="text"
            placeholder="치즈, 우유, 밀가루 등"
    ...
    <div class="mt-16 w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-emerald-50 flex gap-5 hover:border-emerald-200 transition">
        ...
        <div>
          ...
          <p class="text-sm text-gray-500 leading-relaxed">
            HACCP 인증 공장부터 신규 영업 신고 업체까지<br/>
            실시간으로 업데이트되는 DB를 확인하세요.
          </p>
        </div>
      </div>
      ...
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-emerald-50 flex gap-5 hover:border-emerald-200 transition">
        ...
        <div class="w-full">
          <h4 class="font-bold text-lg mb-1 text-gray-800">인기 원재료 트렌드</h4>
          <div class="flex gap-2 mt-2">
            <span class="text-xs bg-emerald-50 text-emerald-700 px-2 py-1 rounded-full font-medium">#식물성치즈</span>
            <span class="text-xs bg-emerald-50 text-emerald-700 px-2 py-1 rounded-full font-medium">#대체당</span>
            <span class="text-xs bg-emerald-50 text-emerald-700 px-2 py-1 rounded-full font-medium">#고단백</span>
          </div>
        </div>
      </div>
    </div>
