<template>
  <div class="min-h-screen bg-[#F9FAFB] dark:bg-[#0F172A] transition-colors duration-300">
    <header class="bg-white dark:bg-[#1E293B] border-b border-gray-100 dark:border-slate-700/50 sticky top-0 z-30 shadow-sm transition-colors">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:h-20 flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-0">
        <NuxtLink to="/" class="text-2xl font-black text-blue-600 dark:text-blue-400 tracking-tighter shrink-0 w-full sm:w-auto text-center sm:text-left transition-colors">
          D-INSIGHT
        </NuxtLink>
        <div class="flex-1 w-full sm:max-w-3xl sm:mx-8">
          <div class="flex flex-col sm:flex-row items-stretch w-full bg-gray-50 dark:bg-[#0F172A] border border-gray-200 dark:border-slate-700/50 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 dark:focus-within:ring-blue-500/50 transition-all shadow-inner sm:h-12">
            ...
            <select
                ...
                @change="refreshData"
            >
              <option class="dark:bg-[#1E293B]" value="축산품">축산품</option>
              <option class="dark:bg-[#1E293B]" value="가공유">가공유</option>
              <option class="dark:bg-[#1E293B]" value="유산균음료">유산균음료</option>
              <option class="dark:bg-[#1E293B]" value="치즈">치즈</option>
              <option class="dark:bg-[#1E293B]" value="유크림">유크림</option>
              <option class="dark:bg-[#1E293B]" value="아이스크림믹스">아이스크림믹스</option>
              <option class="dark:bg-[#1E293B]" value="커피">커피</option>
              <option class="dark:bg-[#1E293B]" value="혼합음료">혼합음료</option>
              <option class="dark:bg-[#1E293B]" value="당류가공품">당류가공품</option>
              <option class="dark:bg-[#1E293B]" value="기타가공품">기타가공품</option>
            </select>
    ...
    <div class="max-w-7xl mx-auto py-6 sm:py-10 px-4 sm:px-6 flex flex-col lg:flex-row gap-6 sm:gap-10">
      <aside class="w-full lg:w-64 shrink-0 hidden lg:block">
        <div class="sticky top-32">
          <h3 class="text-sm font-bold text-gray-400 dark:text-slate-500 mb-4 tracking-widest uppercase flex items-center gap-2 transition-colors">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            최근 검색어
          </h3>
          <div class="bg-white dark:bg-[#1E293B] rounded-2xl p-6 border border-gray-100 dark:border-slate-700/50 shadow-sm transition-colors">
            <ul v-if="recentSearches.length > 0" class="space-y-3">
              <li v-for="(item, index) in recentSearches" :key="index" class="flex items-center justify-between group">
                <span @click="clickRecentSearch(item)" class="text-[13px] font-medium text-gray-600 dark:text-slate-300 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 line-clamp-2 leading-tight transition-colors w-full break-all pr-2" :title="item.label">{{ item.label }}</span>
                <button @click.stop="removeRecentSearch(index)" class="text-gray-300 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 transition-colors shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </li>
            </ul>
            <div v-else class="text-center text-sm text-gray-400 dark:text-slate-500 py-4 transition-colors">최근 검색 기록이 없습니다.</div>
            <div v-if="recentSearches.length > 0" class="mt-4 pt-4 border-t border-gray-50 dark:border-slate-700/50 transition-colors">
              <button @click="clearRecentSearches" class="w-full py-2 text-xs font-bold text-gray-400 dark:text-slate-400 bg-gray-50 dark:bg-slate-800 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 hover:text-gray-600 dark:hover:text-slate-200 transition-colors">기록 전체 삭제</button>
            </div>
          </div>
        </div>
      </aside>
